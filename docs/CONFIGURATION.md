...
`.github/dependabot.yml` is what produces those pull requests. It did not exist until 3.1.0, so the
merge half of the mechanism sat armed while nothing fed it — which is how nineteen packages drifted
behind. npm patch and minor updates arrive **grouped into one weekly PR**; ten separate green PRs a
week teach a reviewer to merge without looking, and that habit is what makes an auto-merged major
dangerous. GitHub Actions are grouped the same way.
...
itself overnight would be a poor way to discover a breaking change.
