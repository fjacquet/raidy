...
1. **IOPS Bruts du Cluster ($IOPS_{raw}$) :** Somme des IOPS de tous les disques.
...
3. **IOPS en Écriture (Write Penalty) :**
   ...
   - Réplication ($R$) :
     $$IOPS_{write} = \frac{IOPS_{raw} \times WriteRatio}{R}$$
...
$$T_{network} = \frac{Bandwidth_{link} \times N_{nodes}}{ReplicationFactor}$$
...
- **Software Overhead :** Ceph ajoute du temps CPU (CRC check, placement CRUSH).
  - _Calcul :_
    $$Latence_{total} \approx (Latence_{media} \times 2) + Latence_{reseau} + Overhead_{cpu}$$
